        ...
        <div class="container">
            <table>
                <colgroup>
                    ...
                    <col style="width: 110px;" v-show="windowWidth >= reactiveMaxWidth" />
                    <col style="width: 180px;" v-show="windowWidth >= reactiveMaxWidth" />
                    <col />
                </colgroup>
                <thead>
                    <tr>
                        <th :colspan="windowWidth >= reactiveMaxWidth ? 4 : 2">
                            <label>지점 검색</label>
                    ...
                    <tr>
                        <th><span>No.</span></th>
                        <th v-show="windowWidth >= reactiveMaxWidth"><span>생산기관</span></th>
                        <th v-show="windowWidth >= reactiveMaxWidth"><span>지점명</span></th>
                        <th v-show="windowWidth >= reactiveMaxWidth"><span>주소</span></th>
                        <th v-show="windowWidth < reactiveMaxWidth"><span>지점정보</span></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td :colspan="windowWidth >= reactiveMaxWidth ? 4 : 2" v-show="searchedStations.length === 0">
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <tr :key="i" v-for="(station, i) in searchedStations" @click="select({ name: station.name, address: station.address })">
                                            <td><span>{{ i + 1 }}</span></td>
                                            <td v-show="windowWidth >= reactiveMaxWidth"><span>{{ station.category }}</span></td>
                                            <td v-show="windowWidth >= reactiveMaxWidth"><span>{{ station.name }}</span></td>
                                            <td v-show="windowWidth >= reactiveMaxWidth"><span>{{ station.address }}</span></td>
                                            <td v-show="windowWidth < reactiveMaxWidth">
                                                <div>
                                                    <h3>{{ station.name }}</h3>
