    ...
    export default {
        ...
        computed: {
            ...mapState({
                ...
                dateTypes: state => state.verification.dateTypes,
                selectedDateType: state => state.verification.selectedDateType,
                available: state => state.verification[state.verification.selectedCategory].available
            })
        },
        methods: {
            ...mapActions({
                setStartDatetime: "SET_VERIFICATION_START_DATETIME",
                setEndDatetime: "SET_VERIFICATION_END_DATETIME",
                setSelectedDateType: "SET_VERIFICATION_SELECTED_DATE_TYPE"
            }),
            ...
            async onChangeStartDatetime(datetime) {
                ...
                else if(this.selectedDateType == "hour" && Math.abs(dayjs(datetime).diff(dayjs(this.endDatetime), "day")) > 30)
            ...
            async onChangeEndDatetime(datetime) {
                ...
                if (datetime < this.startDatetime.format(dayjsToStringFormat))
                    await new Promise(resolve => alert("잘못된 시간대 입력입니다.", resolve));
                else if(this.selectedDateType == "hour" && Math.abs(dayjs(datetime).diff(dayjs(this.startDatetime), "day")) > 30)
                    await new Promise(resolve => alert("검색 가능 기간은 30일입니다.", resolve));
