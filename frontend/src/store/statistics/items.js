...
export default {
    ...
    mutations: {
        SET_STATISTICS_SELECTED_ITEM: (state, item) => state.statistics[state.statistics.selectedCategory].selectedItem = Object.freeze(item),
        SET_STATISTICS_AVAILABLE: (state, { category, available }) => {
            ...
            state.statistics[category].available = availableList.map(datetimes => datetimes.length === 1 ? [datetimes[0]] : [datetimes[0], dayjs(datetimes[datetimes.length - 1].format("YYYYMMDDHH5959"))]);
        }
    },
    actions: {
        SET_STATISTICS_SELECTED_ITEM: (context, item) => context.commit("SET_STATISTICS_SELECTED_ITEM", item),
        SET_STATISTICS_AVAILABLE: (context, { category, available }) => context.commit("SET_STATISTICS_AVAILABLE", { category, available })
    }
}
