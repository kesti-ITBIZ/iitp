...
SELECT
    ...
    AVG(a.o3) AS o3,
    AVG(a.no2) AS no2,
    AVG(a.pm10_qc) AS pm10,
    AVG(a.pm25_qc) AS pm25
FROM "TB_AIRKOREA_DATA_QC" a
...
AND a.stn_nm = '외연도'
AND a.pm10_qc BETWEEN 31 AND 80
AND a.pm25_qc BETWEEN 16 AND 35
GROUP BY
...
SELECT
    ...
    AVG(a.pm25_r_qc) AS pm25
FROM "TB_OBSERVER_DATA_QC" a
JOIN "TB_OBSERVER_STN_RAW" b
ON a.stn_id = b.stn_serial
WHERE a.data_time
    BETWEEN '2020-10-30 00:08:01'
    AND '2020-10-30 02:21:44'
AND b.stn_nm = 'GSN-3'
AND a.pm25_r_qc BETWEEN 16 AND 35
GROUP BY
...
SELECT * FROM (
    SELECT
        ...
        a.stn_id AS std_stn_id,
        a.stn_nm AS std_stn_nm,
        a.pm10_qc AS std_pm10,
        a.pm25_qc AS std_pm25
    FROM "TB_AIRKOREA_DATA_QC" a
    WHERE a."time"
...
JOIN (
    SELECT
        ...
        a.stn_id AS comp_stn_id,
        b.addr AS comp_stn_nm,
        a.pm10_qc AS comp_pm10,
        a.pm25_qc AS comp_pm25
    FROM "TB_SDOT_DATA_QC" a
    JOIN "TB_SDOT_STN_RAW" b
    ON a.stn_id = b.stn_id
    WHERE a.regist_time
